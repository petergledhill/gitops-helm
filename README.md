...
[![Build Status](https://travis-ci.org/petergledhill/gitops-helm.svg?branch=master)](https://travis-ci.org/petergledhill/gitops-helm)
...
![gitops](https://github.com/petergledhill/openfaas-flux/blob/master/docs/screens/flux-helm-gitops.png)
...
You can fork the [gitops-helm](https://github.com/petergledhill/gitops-helm) project and use it as a template for your cluster config.
...
(replace `petergledhill` with your GitHub username): 

```bash
helm install --name flux \
--set rbac.create=true \
--set helmOperator.create=true \
--set git.url=git@github.com:petergledhill/gitops-helm \
--namespace flux \
weaveworks/flux
```
...
I will be using [podinfo](https://github.com/petergledhill/k8s-podinfo) to demonstrate a full CI/CD pipeline including promoting releases between environments.  
...
Let's create an image corresponding to the `dev` branch (replace `petergledhill` with your Docker Hub username):

```
$ cd hack && ./ci-mock.sh -r petergledhill/podinfo -b dev
...
....
Step 12/15 : COPY --from=builder /go/src/github.com/petergledhill/k8s-podinfo/podinfo .
....
Step 15/15 : CMD ["./podinfo"]
....
Successfully built 71bee4549fb2
Successfully tagged petergledhill/podinfo:dev-kb9lm91e
The push refers to repository [docker.io/petergledhill/podinfo]
36ced78d2ca2: Pushed 
```
...
spec:
  ...
  chart:
    git: git@github.com:petergledhill/gitops-helm
    path: charts/podinfo
    ref: master
  values:
    image: petergledhill/podinfo:dev-kb9lm91e
    replicaCount: 1
```
...
![gitops-automation](https://github.com/petergledhill/openfaas-flux/blob/master/docs/screens/flux-helm-image-update.png)
...
![gitops-chart-change](https://github.com/petergledhill/openfaas-flux/blob/master/docs/screens/flux-helm-chart-update.png)
...
```bash
$ cd hack && ./ci-mock.sh -r petergledhill/podinfo -b stg

Successfully tagged petergledhill/podinfo:stg-9ij63o4c
The push refers to repository [docker.io/petergledhill/podinfo]
8f21c3669055: Pushed 
```
...
spec:
  ...
  chart:
    ...
    ref: master
  values:
    image: petergledhill/podinfo:stg-9ij63o4c
    replicaCount: 2
    hpa:
      enabled: true
      maxReplicas: 10
...
* make the CI tooling publish images from my hotfix branch to `petergledhill/podinfo:hotfix-sha`
...
```bash
$ cd hack && ./ci-mock.sh -r petergledhill/podinfo -v 0.4.10

Successfully built f176482168f8
Successfully tagged petergledhill/podinfo:0.4.10
``` 
...
spec:
  ...
  chart:
    ...
    ref: master
  values:
    image: petergledhill/podinfo:0.4.10
    replicaCount: 3
```
...
```bash
$ cd hack && ./ci-mock.sh -r petergledhill/podinfo -v 0.4.11

Successfully tagged petergledhill/podinfo:0.4.11
```

![gitops-semver](https://github.com/petergledhill/openfaas-flux/blob/master/docs/screens/flux-helm-semver.png)
...
![SealedSecrets](https://github.com/petergledhill/openfaas-flux/blob/master/docs/screens/flux-secrets.png)
